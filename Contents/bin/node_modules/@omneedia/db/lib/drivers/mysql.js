    ...
    query: function (name, sql, fn, x) {
        this.connect(name, function (err, q) {
            if (err) fn('CONNECTION_REFUSED', err);
                ...
                if (x) q.query(sql, fn, function (err, rows, fields) {
                    q.end();
                    x(err, rows);
                });
                else q.query(sql, function (err, rows, fields) {
                    q.end();
                    fn(err, rows);
                });
            };
    ...
    store: function (name, sql, fn, o) {
        ...
        this.connect(name, function (err, q) {
            if (err) fn('CONNECTION_REFUSED', err);
                ...
                if (o) sql = sql + ' limit ' + o.start + ',' + o.limit;
                q.query(sql, function (err, rows, fields) {
                    if (!err) {
                                        ...
                                        if (typeof o[el] === 'object') values.push(qstr(JSON.stringify(o[el])));
                                            ...
                                            try {
                                                if (o[el].toLowerCase() == "null") values.push(o[el].toUpperCase());
                                                    ...
                                                    if (o[el].indexOf('{{') == 0) values.push('(' + o[el].split('{{')[1].split('}}')[0] + ')');
